...
export default {
  ...
  methods: {
    ...
    check_mobile() {
      ...
      // 发送axios请求，判断手机号是否存在.携带参数 {params:{k:v}}
      ...
      if (this.username && this.password) {
        ...
        this.$axios.post(this.$settings.base_url + '/user/login/', {
          ...
          if(response.data.code === 1){
            ...
            // 用户不存在抛出异常，由catch捕捉
            let msg = Object.values(response.data.msg).reduce((prev,cur)=>{
              return prev+cur;
            },"")
            throw new Error(msg)
          }
